...
export default function RefItemBlock({ blockIdx, sectionIdx }: BlockComponentProps) {
    ...
    const refOptions = Object.entries(ERefRelation).map(([key, value]) => ({
        value,
        label: key
    }));

    const orderOptions = Object.entries(EOrder).map(([key, value]) => ({
        value,
        label: key
    }));
        ...
        <section className="space-y-6">
            ...
            {Array.isArray(selected) && (
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel>Choose Specific Ones</FormLabel>
                            <FormControl>
                                <InfiniteMultiSelect
                                    endpoint={`${form.watch(`${blockName}.ref`)}/options`}
                                    onSelectionChange={val => {
